...
void RigidBody::calculateDerivedData()
{
    ...
    _calculateTransformMatrix(transform, position, orientation);

    _transformInertiaTensor(inverseInertiaWorld, orientation, inverseInertia, transform);
}
...
Vector3 RigidBody::toWorldSpace(const Vector3& bodyPoint)
{
    return transform.transform(bodyPoint);
}

Vector3 RigidBody::toBodySpace(const Vector3& worldPoint)
{
    return transform.inverse().transform(worldPoint);
}
